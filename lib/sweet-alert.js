...
(function() {
    ...
    window.onkeydown = function(e) {
      ...
      if (modalIsVisible) {
        ...
        if (keycode === 9) {
          var focusIndex = -1,
              nextFocus = 0,
              $modalButtons = modal.querySelectorAll('button:not(.hidden)'),
              i = 0;

          for (i; i < $modalButtons.length; i++) {
              if ($modalButtons[i] === document.activeElement) {
                  focusIndex = i;
                  nextFocus = (i+1 >= $modalButtons.length) ? 0 : i+1;
              }
          }
          $modalButtons[nextFocus].focus();
          e.preventDefault();
        }
        // Handle ENTER / SPACE
  ...
  // Give focus to the first non-hidden button
  function setFocusedButton() {
    var modal = getModal();
    modal.querySelectorAll('button:not(.hidden)')[0].focus();
    console.log(modal.querySelectorAll('button:not(.hidden)')[0]);
