...
QUnit.test( "GO CANUCKS GO!", function( assert ) {
	swal({
		title: "GO CANUCKS GO!",
		videoUrl: 'http://www.youtube.com/embed/pvG73bCcGVc'
	});
	var alert = document.querySelector(".sweet-alert"),
		text = alert.getElementsByTagName("h2")[0].innerHTML,
		videoUrl = alert.querySelector(".videoUrl"),
		okButton = alert.getElementsByTagName("button")[1],
		
	assert.equal(text, "GO CANUCKS GO!");
	assert.equal(videoUrl, 'http://www.youtube.com/embed/pvG73bCcGVc');
	$(okButton).trigger("click");
});
